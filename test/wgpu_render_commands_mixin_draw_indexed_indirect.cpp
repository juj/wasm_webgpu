// Verifies that wgpu_render_commands_mixin_draw_indexed_indirect() draws using indexed draw arguments sourced from an indirect buffer, with a uint16 index buffer bound.
...
void ObtainedWebGpuDevice(WGpuDevice device, void *userData)
{
  ...
  // Create an index buffer with 3 uint16 indices
  ...
  // Indirect buffer: [indexCount=3, instanceCount=1, firstIndex=0, baseVertex=0, firstInstance=0]
  uint32_t indirectArgs[] = { 3, 1, 0, 0, 0 };
  ...
  wgpu_render_commands_mixin_draw_indexed_indirect(pass, indirectBuf, 0);
