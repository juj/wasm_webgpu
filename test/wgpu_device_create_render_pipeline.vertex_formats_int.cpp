// Tests creating render pipelines using integer vertex attribute formats
// (UINT8X2, UINT16X2, UINT32X2, SINT8X2, SINT16X2, SINT32X2) to exercise
// GPUTextureAndVertexFormats indices 103, 115, 134, 106, 118, 138 in the
// marshalling layer.
...
static const FmtCase kCases[] = {
  // Unsigned integer formats: shader input type is vec2u
  { WGPU_VERTEX_FORMAT_UINT8X2,  "vec2u", "uint8x2"  },  // index 103
  { WGPU_VERTEX_FORMAT_UINT16X2, "vec2u", "uint16x2" },  // index 115
  { WGPU_VERTEX_FORMAT_UINT32X2, "vec2u", "uint32x2" },  // index 134
  // Signed integer formats: shader input type is vec2i
  { WGPU_VERTEX_FORMAT_SINT8X2,  "vec2i", "sint8x2"  },  // index 106
  { WGPU_VERTEX_FORMAT_SINT16X2, "vec2i", "sint16x2" },  // index 118
  { WGPU_VERTEX_FORMAT_SINT32X2, "vec2i", "sint32x2" },  // index 138
};

static void onError(WGpuDevice d, WGPU_ERROR_TYPE t, const char *m, void *u)
{
  if (t != WGPU_ERROR_TYPE_NO_ERROR)
    printf("Int vertex format %s: error %s\n", kCases[step-1].name, m ? m : "");
  tryNext(u);
}
...
static void tryNext(void *userData)
{
  ...
  printf("Testing integer vertex format: %s\n", c->name);

  // Build WGSL with the correct integer component type at location 0
  char shaderBuf[512];
  snprintf(shaderBuf, sizeof(shaderBuf),
    "@vertex fn vs(@location(0) a: %s) -> @builtin(position) vec4f {"
    "  return vec4f(f32(a.x), f32(a.y), 0, 1); }"
  ...
  vbl.arrayStride = 4; // 2 × 2-byte components (conservative stride)
  vbl.attributes = &attr;
  vbl.numAttributes = 1;
