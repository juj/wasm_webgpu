// Tests creating a render pipeline with zero vertex buffer layouts
// (numBuffers == 0), exercising the 'while(numVertexBuffers--)' zero-iteration
// path in $wgpuReadRenderPipelineDescriptor that results in an empty buffers[].
...
void ObtainedWebGpuDevice(WGpuDevice device, void *userData)
{
  ...
  desc.vertex.buffers    = 0;    // no vertex buffers — zero-iteration loop
  desc.vertex.numBuffers = 0;
