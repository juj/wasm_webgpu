// Verifies that wgpu_is_render_commands_mixin() returns true for a WGpuRenderPassEncoder and false for a WGpuCommandEncoder.
...
void ObtainedWebGpuDevice(WGpuDevice device, void *userData)
{
  ...
  assert(pass);
  assert(wgpu_is_render_commands_mixin(pass));
  assert(!wgpu_is_render_commands_mixin(encoder)); // a command encoder is not a render commands mixin
