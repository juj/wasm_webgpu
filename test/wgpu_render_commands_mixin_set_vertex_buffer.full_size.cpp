// Tests wgpu_render_commands_mixin_set_vertex_buffer() with size=WGPU_MAX_SIZE,
// exercising the 'size < 0 ? void 0 : size' false branch (void 0 / full-buffer
// path). The existing .cpp and sub_size.cpp tests both pass a non-negative size,
// so neither reaches the void 0 branch. Here we pass WGPU_MAX_SIZE (negative
// sentinel) so the browser binds from offset to the end of the buffer.
...
void ObtainedWebGpuDevice(WGpuDevice device, void *userData)
{
  ...
  // Pass WGPU_MAX_SIZE (negative) so the JS takes the 'size < 0 ? void 0 : size'
  // false branch and calls setVertexBuffer() with undefined size (bind to end).
  wgpu_render_commands_mixin_set_vertex_buffer(pass, 0, vbuf, 0, WGPU_MAX_SIZE);
