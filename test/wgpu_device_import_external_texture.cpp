// Verifies wgpu_device_import_external_texture() and wgpu_is_external_texture().
...
// (readyState >= HAVE_CURRENT_DATA) before importing, matching the WebGPU spec requirement.
...
WGPU_BOOL PollVideoReady(double time, void *userData)
{
  ...
  assert(!wgpu_is_texture(ext)); // an external texture is not a regular texture
