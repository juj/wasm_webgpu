// Tests creating a render pipeline with TRIANGLE_STRIP topology and a
// stripIndexFormat, exercising the GPUIndexFormats code path in
// wgpuReadRenderPipelineDescriptor (primitiveIdx+1).
...
void ObtainedWebGpuDevice(WGpuDevice device, void *userData)
{
  ...
  // Use TRIANGLE_STRIP topology with uint32 strip index format.
  desc.primitive.topology = WGPU_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP;
  desc.primitive.stripIndexFormat = WGPU_INDEX_FORMAT_UINT32;
  ...
  wgpu_render_commands_mixin_draw(pass, 4, 1, 0, 0);
