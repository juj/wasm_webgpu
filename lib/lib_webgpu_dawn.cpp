...
const WGPUTextureFormat WGPU_TEXTURE_FORMAT_to_Dawn[] = {
  ...
  WGPUTextureFormat_RGB10A2Unorm, // WGPU_TEXTURE_FORMAT_RGB10A2UNORM 26
  WGPUTextureFormat_RG11B10Ufloat, // WGPU_TEXTURE_FORMAT_RG11B10UFLOAT 27
  // 64-bit formats
  WGPUTextureFormat_RG32Uint, // WGPU_TEXTURE_FORMAT_RG32UINT 28
  WGPUTextureFormat_RG32Sint, // WGPU_TEXTURE_FORMAT_RG32SINT 29
  WGPUTextureFormat_RG32Float, // WGPU_TEXTURE_FORMAT_RG32FLOAT 30     
  WGPUTextureFormat_RGBA16Uint, // WGPU_TEXTURE_FORMAT_RGBA16UINT 31
  WGPUTextureFormat_RGBA16Sint, // WGPU_TEXTURE_FORMAT_RGBA16SINT 32
  WGPUTextureFormat_RGBA16Float, // WGPU_TEXTURE_FORMAT_RGBA16FLOAT 33
  // 128-bit formats
  WGPUTextureFormat_RGBA32Uint, // WGPU_TEXTURE_FORMAT_RGBA32UINT 34
  WGPUTextureFormat_RGBA32Sint, // WGPU_TEXTURE_FORMAT_RGBA32SINT 35
  WGPUTextureFormat_RGBA32Float, // WGPU_TEXTURE_FORMAT_RGBA32FLOAT 36
  // Depth/stencil formats
  WGPUTextureFormat_Stencil8, // WGPU_TEXTURE_FORMAT_STENCIL8 37
...
WGPU_DEVICE_LOST_REASON Dawn_to_WGPU_DEVICE_LOST_REASON[] = {
  WGPU_DEVICE_LOST_REASON_INVALID,
...
WGpuAdapter navigator_gpu_request_adapter_sync(const WGpuRequestAdapterOptions* options) {
  ...
  dawn::native::Instance& instance = GetDawnInstance();

  instance.DiscoverDefaultAdapters();
  instance.EnableBeginCaptureOnStartup(true);

  dawn::native::Adapter finalAdapter;
  ...
  for (auto adapter : adapters) {
    ...
    if (properties.backendType < WGPUBackendType_D3D12 || properties.adapterType > WGPUAdapterType_IntegratedGPU)
      continue;
    ...
    // their associated platforms.
    if (properties.backendType > adapterProperties.backendType)
      continue;

    if (options && options->powerPreference == WGPU_POWER_PREFERENCE_LOW_POWER) {
      if (adapterProperties.adapterType == WGPUAdapterType_IntegratedGPU) {
        finalAdapter = adapter;
        adapterProperties = properties;
        continue;
      }
    } else if (properties.adapterType == WGPUAdapterType_DiscreteGPU) {
      finalAdapter = adapter;
      adapterProperties = properties;
      continue;
    }
...
void wgpu_adapter_or_device_get_limits(WGpuAdapter adapterOrDevice, WGpuSupportedLimits* limits) {
  ...
  //limits->maxColorAttachmentBytesPerSample = _limits.limits.maxColorAttachmentBytesPerSample; // not in dawn yet?
...
void wgpu_device_create_compute_pipeline_async(WGpuDevice device, WGpuShaderModule computeModule, const char* entryPoint, WGpuPipelineLayout layout,
  ...
  wgpuDeviceCreateComputePipelineAsync(_wgpu_get_dawn<WGPUDevice>(device), &_desc,
    [](WGPUCreatePipelineAsyncStatus status, WGPUComputePipeline pipeline, char const* message, void* userdata) {
      ...
      data->callback(data->device, _pipeline, data->userData);
      delete data;
    }, data);
}
...
WGpuRenderBundleEncoder wgpu_device_create_render_bundle_encoder(WGpuDevice device, const WGpuRenderBundleEncoderDescriptor *renderBundleEncoderDesc) {
  ...
  _desc.colorFormatsCount = (uint32_t)renderBundleEncoderDesc->numColorFormats;
...
WGpuQuerySet wgpu_device_create_query_set(WGpuDevice device, const WGpuQuerySetDescriptor *querySetDesc) {
  ...
  _desc.pipelineStatisticsCount = 0;
...
WGpuComputePassEncoder wgpu_command_encoder_begin_compute_pass(WGpuCommandEncoder commandEncoder, const WGpuComputePassDescriptor *computePassDesc) {
  ...
  std::vector<WGPUComputePassTimestampWrite> timestampWrites(computePassDesc ? computePassDesc->numTimestampWrites : 0);
  if (computePassDesc != nullptr) {
    _desc.timestampWriteCount = computePassDesc ? computePassDesc->numTimestampWrites : 0;
    for (int i = 0; i < computePassDesc->numTimestampWrites; ++i) {
      timestampWrites[i].querySet = _wgpu_get_dawn<WGPUQuerySet>(computePassDesc->timestampWrites[i].querySet);
      timestampWrites[i].queryIndex = computePassDesc->timestampWrites[i].queryIndex;
      timestampWrites[i].location = wgpu_compute_pass_timestamp_location_to_dawn(computePassDesc->timestampWrites[i].location);
    }
    _desc.timestampWrites = timestampWrites.data();
  }
