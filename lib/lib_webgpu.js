...
//  navigator_gpu_request_adapter_sync__sig: 'ip', // Iirc this would be needed for -sASYNCIFY=1 build mode, but this breaks -sMEMORY64=1 due to the detrimental automatic wrappers
...
#endif
    var infoByteIdx = {{{ shiftPtr('infoPtr') }}};
...
#if MEMORY64
    HEAPU64[infoIdx + 256n] = adapterInfo['subgroupMinSize'];
    HEAPU64[infoIdx + 257n] = adapterInfo['subgroupMaxSize'];
    HEAPU64[infoIdx + 258n] = adapterInfo['isFallbackAdapter'];
#else
  ...
  wgpu_device_create_compute_pipeline_async: function(device, computeModule, entryPoint, layout, constants, numConstants, callback, userData) {
    ...
    let cb = (pipeline) => {
      ...
      {{{ makeDynCall('vipip', 'callback') }}}(device, /*error*/ 0, wgpuStoreAndSetParent(pipeline, deviceObject), userData);
    };
