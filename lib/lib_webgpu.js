...
{{{
  ...
  globalThis.ptrToIdx = function(ptr, shift) {
    ...
    if (ASSERTIONS || parseInt(globalThis.WEBGPU_DEBUG)) {
      if (MEMORY64) s += `assert((${ptr} >> ${shift}n) << ${shift}n == ${ptr});\n`;
      else          s += `assert((${ptr} >> ${shift})  << ${shift}  == ${ptr});\n`;
    }
    if (MEMORY64) s += `${ptr} = Number(${ptr} >> ${shift}n);`;
    else if (MAXIMUM_MEMORY > 2*1024*1024*1024) s += `${ptr} >>>= ${shift};`;
    else s += `${ptr} >>= ${shift};`;
    return s;
  }
  globalThis.shiftPtr = function(ptr, shift) {
    if (MEMORY64) return `Number(${ptr} >> ${shift}n)`;
    if (MAXIMUM_MEMORY > 2*1024*1024*1024) return `(${ptr} >>> ${shift})`;
    return `(${ptr} >> ${shift})`;
  }
  }
  null;
...
let api = {
  ...
  $wgpuStoreAndSetParent: function(object, parent) {
    if (object) {
      ...
      object.parentObject = parent; // Link child->parent
      parent.derivedObjects[objectId] = object; // Link parent->child
  ...
  wgpu_canvas_get_webgpu_context: function(canvasSelector) {
    ...
    let canvas = GL.offscreenCanvases[UTF8ToString(canvasSelector)].offscreenCanvas;
#else
    ...
    ctx.derivedObjects = {}; // Current texture views are derived off of CanvasContext object.
...
#if ASYNCIFY == 2
    cb = WebAssembly.promising({{{ makeDynCall('idp', 'cb') }}});
#else
...
//////////////////////////////////////////////////////////////////
  ...
  wgpu_canvas_context_configure: function(canvasContext, config) {
    ...
    let desc = {
      ...
      'viewFormats': wgpuReadArrayOfWgpuObjects(HEAPU32[config+4], HEAPU32[config+3]), // TODO: Wasm64
  ...
  wgpu_canvas_context_get_current_texture: function(canvasContext) {
    ...
    if (canvasTexture != wgpu[1]) {
      ...
      canvasTexture.wid = 1;
      canvasTexture.derivedObjects = {}; // GPUTextureViews are derived off of GPUTextures
      canvasTexture.parentObject = canvasContext; // Link child -> parent
      canvasContext.derivedObjects[1] = canvasTexture; // Link parent -> child
    }
  ...
  $wgpuReadQueueDescriptor: function(heap32Idx) {
    return HEAPU32[heap32Idx] ? { 'label': UTF8ToString(HEAPU32[heap32Idx]) } : void 0;
  },
...
#if ASYNCIFY
  wgpu_adapter_request_device_sync__deps: ['$wgpuStore', '$wgpuStoreAsyncifiedOp', '$wgpuReadDeviceDescriptor', '_wgpuNumAsyncifiedOperationsPending', '$wgpu_async'],
...
#if ASYNCIFY
  wgpu_adapter_request_device_sync_simple__deps: ['$wgpuStore', '$wgpuStoreAsyncifiedOp', '_wgpuNumAsyncifiedOperationsPending', '$wgpu_async'],
  ...
  $wgpuReadShaderModuleCompilationHints: function(index) {
    let numHints = HEAP32[index+2],
      hints = [],
      hintsIndex = {{{ shiftPtr('HEAPU32[index]', 2) }}}, // TODO: Wasm64
      layout;
    {{{ wassert('numHints >= 0'); }}}
    while(numHints--) {
      ...
      hints.push({
        'entryPoint': UTF8ToString(HEAPU32[hintsIndex]), // TODO: Wasm64
  ...
  $wgpuReadRenderPipelineDescriptor: function(descriptor) {
    ...
    let vertexBuffers = [],
        ...
        vertexBuffersIdx = {{{ shiftPtr('HEAPU32[vertexIdx+2]', 2) }}}, // +2 == WGpuVertexState.buffers // TODO: Wasm64
        ...
        targetsIdx = {{{ shiftPtr('HEAPU32[fragmentIdx+2]', 2) }}}, // +2 == WGpuFragmentState.targets // TODO: Wasm64
    ...
    while(numVertexBuffers--) {
      let attributes = [],
          ...
          attributesIdx = {{{ shiftPtr('HEAPU32[vertexBuffersIdx]', 2) }}}; // TODO: Wasm64
    ...
    desc = {
      'vertex': {
        ...
        // If null pointer was passed to use the default entry point name, then UTF8ToString() would return '', but spec requires undefined.
        'entryPoint': UTF8ToString(HEAPU32[vertexIdx]) || void 0, // TODO: Wasm64
        'buffers': vertexBuffers,
        'constants': wgpuReadConstants(HEAPU32[vertexIdx+4], HEAP32[vertexIdx+8]) // TODO: Wasm64
      },
      'fragment': fragmentModule ? {
        'module': wgpu[fragmentModule],
        // If null pointer was passed to use the default entry point name, then UTF8ToString() would return '', but spec requires undefined.
        'entryPoint': UTF8ToString(HEAPU32[fragmentIdx]) || void 0, // TODO: Wasm64
        'targets': targets,
        'constants': wgpuReadConstants(HEAPU32[fragmentIdx+4], HEAP32[fragmentIdx+8]) // TODO: Wasm64
      } : void 0,
      'primitive': {
...
#if ASYNCIFY
  wgpu_buffer_map_sync__deps: ['_wgpuNumAsyncifiedOperationsPending', '$wgpuStoreAsyncifiedOp', '$wgpu_async'],
  ...
  wgpu_buffer_map_sync: function(buffer, mode, offset, size) {
    return wgpu_async(() => {
      ...
      return wgpu[buffer]['mapAsync'](mode, offset, size < 0 ? void 0 : size)
  ...
  wgpu_device_create_texture: function(device, descriptor) {
    ...
    let desc = {
      'viewFormats': wgpuReadArrayOfWgpuObjects(HEAPU32[descriptor], HEAPU32[descriptor+2]), // TODO: Wasm64
  ...
  $wgpuReadConstants: function(constants, numConstants) {
    ...
    let c = {};
    while(numConstants--) {
      c[UTF8ToString(HEAPU32[{{{ shiftPtr('constants', 2) }}}])] = HEAPF64[{{{ shiftPtr('constants + 8', 3) }}}]; // TODO Wasm64
      constants += 16;
    }
    return c;
  },
  ...
  wgpu_command_encoder_begin_render_pass: function(commandEncoder, descriptor) {
    ...
    let colorAttachments = [],
      ...
      colorAttachmentsIdx = {{{ shiftPtr('HEAPU32[descriptor+2]', 2) }}}, // TODO: Wasm64
  ...
  wgpu_command_encoder_begin_render_pass_1color_0depth: function(commandEncoder, descriptor) {
    ...
    let colorAttachmentsIdx = {{{ shiftPtr('HEAPU32[descriptor+2]', 2) }}},
      colorAttachmentsIdxDbl = {{{ shiftPtr('colorAttachmentsIdx + 6', 1) }}}; // Alias the view for HEAPF64.
  ...
  canvas_transfer_control_to_offscreen__deps: ['$wgpuOffscreenCanvases'],
  canvas_transfer_control_to_offscreen: function(canvasSelector, offscreenCanvasId) {
    {{{ wdebuglog('`canvas_transfer_control_to_offscreen(canvasSelector="${UTF8ToString(canvasSelector)}", offscreenCanvasId=${offscreenCanvasId})`'); }}}
    {{{ wassert('canvasSelector'); }}}
    {{{ wassert('offscreenCanvasId'); }}}
    {{{ wassert('!wgpuOffscreenCanvases[offscreenCanvasId]'); }}}
  ...
  offscreen_canvas_size: function(offscreenCanvasId, outWidth, outHeight) {
    ...
    {{{ wassert('outWidth'); }}}
    {{{ wassert('outHeight'); }}}
    {{{ wassert('outWidth % 4 == 0'); }}} // Must be aligned at uint32_t boundary
    {{{ wassert('outHeight % 4 == 0'); }}} // Must be aligned at uint32_t boundary
