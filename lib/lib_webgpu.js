...
// wgpuFeatures: 'depth-clip-control depth24unorm-stencil8 depth32float-stencil8 texture-compression-bc texture-compression-etc2 texture-compression-astc timestamp-query indirect-first-instance'.split(' '),
  wgpuFeatures__deps: ['$wgpuDecodeStrings'],
  wgpuFeatures: "wgpuDecodeStrings('C-clip-control C24unormB C32floatBDbcDetc2DaAc timeAamp-query indirect-firA-inAance',' texture-compression-|depth|-stencil8|st').slice(1)",
...
//  $GPUTextureSampleTypes: [, 'float', 'unfilterable-float', 'depth', 'sint', 'uint'],
  ...
  wgpuDispatchWebGpuErrorEvent: function(device, callback, error, userData) {
    ...
    _wgpuReportErrorCodeAndMessage(device,
      callback,
      error ? (error instanceof GPUValidationError
  ...
  wgpu_adapter_request_device_async: function(adapter, descriptor, deviceCallback, userData) {
    ...
    let requiredFeatures = [], requiredLimits = {}, v = HEAPU32[descriptor++];
    ...
    for(let i = 0; i < 8/*_wgpuFeatures.length*/; ++i) {
  ...
  $wgpuReadShaderModuleCompilationHints: function(index) {
    ...
    let hints = {};
    let hintsIndex = {{{ shiftPtr('HEAPU32[index+1]', 2) }}};
    while(numHints--) {
      hints[UTF8ToString(HEAPU32[hintsIndex])] = wgpu[HEAPU32[hintsIndex+1]];
      hintsIndex += 2;
    }
    return hints;
  },

  wgpu_device_create_shader_module__deps: ['$wgpuStoreAndSetParent', '$wgpuReadShaderModuleCompilationHints'],
  ...
  wgpu_device_create_query_set: function(device, descriptor) { // TODO: this function is untested. Write a test case
    ...
    return wgpuStoreAndSetParent(
      device['createQuerySet'](
        debugDir(
          {
            'type': [, 'occlusion'][HEAPU32[descriptor]],
  ...
  wgpu_command_encoder_finish: function(commandEncoder) {
    ...
    let cmdBuffer = wgpu[commandEncoder]['finish']();
    {{{ wdebuglog('`returned:`'); }}}
    {{{ wdebugdir('cmdBuffer'); }}}
  ...
  wgpu_compute_pass_encoder_dispatch: function(encoder, workgroupCountX, workgroupCountY, workgroupCountZ) { // TODO: this function is untested. Write a test case
    {{{ wdebuglog('`wgpu_compute_pass_encoder_dispatch(encoder=${encoder}, workgroupCountX=${workgroupCountX}, workgroupCountY=${workgroupCountY}, workgroupCountZ=${workgroupCountZ})`'); }}}
    {{{ wassert('wgpu[encoder] instanceof GPUComputePassEncoder'); }}}
    wgpu[encoder]['dispatch'](workgroupCountX, workgroupCountY, workgroupCountZ);
  },
  ...
  wgpu_queue_write_buffer: function(queue, buffer, bufferOffset, data, size) { // TODO: this function is untested. Write a test case
    ...
    // Work around Chrome performance bug https://bugs.chromium.org/p/chromium/issues/detail?id=1134457
    let tempView = new Uint8Array(HEAPU8.buffer, data, size);
    wgpu[queue]['writeBuffer'](wgpu[buffer], bufferOffset, tempView, 0, size);
    // XXX migrate back to:
//    wgpu[queue]['writeBuffer'](wgpu[buffer], bufferOffset, HEAPU8, data, size);
  ...
  wgpu_queue_write_texture: function(queue, destination, data, bytesPerBlockRow, blockRowsPerImage, writeWidth, writeHeight, writeDepthOrArrayLayers) { // TODO: this function is untested. Write a test case
    ...
    // Work around Chrome performance bug https://bugs.chromium.org/p/chromium/issues/detail?id=1134457
    let tempView = new Uint8Array(HEAPU8.buffer, data, bytesPerBlockRow*blockRowsPerImage);
    wgpu[queue]['writeTexture'](wgpuReadGpuImageCopyTexture(destination), tempView, { 'offset': 0, 'bytesPerRow': bytesPerBlockRow, 'rowsPerImage': blockRowsPerImage }, [writeWidth, writeHeight, writeDepthOrArrayLayers]);
    // XXX migrate back to:
