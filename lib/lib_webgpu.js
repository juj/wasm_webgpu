...
/*
  ...
  $llalloc__deps: ['$llnodes'],
  $llalloc: function(key,value) {
    var n = llnodes[0] ? llnodes.pop() : {};
    n.key = key;
    n.value = value;
    return n;
  },

  $llfree__deps: ['$llnodes'],
  $llfree: function(node) {
    node.value = 0;
    llnodes.push(node);
  },

  $llinsert: function(node, list) {
    node.next = list;
    return node;
  },
*/
  $mappool: [],
  $newMap__deps: ['$mappool'],
  $newMap: function() {
    return mappool[0] ? mappool.pop() : new Map();
  },

  $deleteMap__deps: ['$mappool'],
  ...
  $wgpuLinkParentAndChild__deps: ['$newMap'],
...
//    if (!parent.derivedObjects) parent.derivedObjects = new Map();
    if (!parent.derivedObjects) parent.derivedObjects = newMap();
    parent.derivedObjects.set(childId, child); // Link parent->child
/////////    parent.derivedObjects = llinsert(llalloc(childId, child), parent.derivedObjects);
//    if (!parent.derivedObjects) parent.derivedObjects = llalloc(childId, child);
//    else llinsert(llalloc(childId, child), parent.derivedObjects);
  },
  ...
  wgpu_object_destroy__deps: ['$wgpu', '$deleteMap'],
...
//      if (o.derivedObjects) o.derivedObjects.forEach((v, k) => { _wgpu_object_destroy(k); });
      if (o.derivedObjects) for(var k of o.derivedObjects.keys()) _wgpu_object_destroy(k);
//      if (o.derivedObjects) for(var k in o.derivedObjects) /*if (o.derivedObjects.hasOwnProperty(k))*/ _wgpu_object_destroy(k);
...
//  navigator_gpu_request_adapter_sync__sig: 'ip', // Iirc this would be needed for -sASYNCIFY=1 build mode, but this breaks -sMEMORY64=1 due to the detrimental automatic wrappers
  ...
  $wgpuReadConstants: function(constants, numConstants) {
    ...
    while(numConstants--) {
      c[utf8({{{ readPtr('constants', 3) }}})] = 
        HEAPF64[{{{ shiftPtr('constants + ' + toWasm64('8'), 3) }}}];
      constants += 16;
    }
    return c;
  },
  ...
  $wgpuReadTimestampWrites_desc: {},
  $wgpuReadTimestampWrites__deps: ['$wgpuReadTimestampWrites_desc'],
  $wgpuReadTimestampWrites: function(timestampWritesIndex) {
    let querySet = HEAPU32[timestampWritesIndex], i;
    if (querySet) {
      wgpuReadTimestampWrites_desc['querySet'] = wgpu[querySet];
      wgpuReadTimestampWrites_desc['beginningOfPassWriteIndex'] = (i = HEAP32[timestampWritesIndex+1]) >= 0 ? i : void 0;
      wgpuReadTimestampWrites_desc['endOfPassWriteIndex'] = (i = HEAP32[timestampWritesIndex+2]) >= 0 ? i : void 0;
      return wgpuReadTimestampWrites_desc;
    }
  },

  $wgpuReadRenderPassDepthStencilAttachment_desc: {},
  $wgpuReadRenderPassDepthStencilAttachment__deps: ['$wgpuReadRenderPassDepthStencilAttachment_desc'],
  $wgpuReadRenderPassDepthStencilAttachment: function(heap32Idx) {
    if (HEAPU32[heap32Idx]) {
      wgpuReadRenderPassDepthStencilAttachment_desc['view'] = wgpu[HEAPU32[heap32Idx]];
      wgpuReadRenderPassDepthStencilAttachment_desc['depthLoadOp'] = GPULoadOps[HEAPU32[heap32Idx+1]];
      wgpuReadRenderPassDepthStencilAttachment_desc['depthClearValue'] = HEAPF32[heap32Idx+2];
      wgpuReadRenderPassDepthStencilAttachment_desc['depthStoreOp'] = GPUStoreOps[HEAPU32[heap32Idx+3]];
      wgpuReadRenderPassDepthStencilAttachment_desc['depthReadOnly'] = !!HEAPU32[heap32Idx+4];
      wgpuReadRenderPassDepthStencilAttachment_desc['stencilLoadOp'] = GPULoadOps[HEAPU32[heap32Idx+5]];
      wgpuReadRenderPassDepthStencilAttachment_desc['stencilClearValue'] = HEAPU32[heap32Idx+6];
      wgpuReadRenderPassDepthStencilAttachment_desc['stencilStoreOp'] = GPUStoreOps[HEAPU32[heap32Idx+7]];
      wgpuReadRenderPassDepthStencilAttachment_desc['stencilReadOnly'] = !!HEAPU32[heap32Idx+8];
      return wgpuReadRenderPassDepthStencilAttachment_desc;
    }
    /*
...
*/
  ...
  $wgpu_command_encoder_begin_render_pass_desc: {},
  $wgpu_command_encoder_begin_render_pass_colorAttachments: [],
  $wgpu_command_encoder_begin_render_pass_colorAttachments_desc: [],
  wgpu_command_encoder_begin_render_pass__deps: ['$GPULoadOps', '$GPUStoreOps', '$wgpuReadTimestampWrites', '$wgpuReadRenderPassDepthStencilAttachment', '$wgpu_command_encoder_begin_render_pass_desc', '$wgpu_command_encoder_begin_render_pass_colorAttachments', '$wgpu_command_encoder_begin_render_pass_colorAttachments_desc'],
  wgpu_command_encoder_begin_render_pass: function(commandEncoder, descriptor) {
    ...
    while (numColorAttachments >= wgpu_command_encoder_begin_render_pass_colorAttachments.length) {
      wgpu_command_encoder_begin_render_pass_colorAttachments.push(new Array(wgpu_command_encoder_begin_render_pass_colorAttachments.length));
      wgpu_command_encoder_begin_render_pass_colorAttachments_desc.push({'clearValue':[0,0,0,0]});
    }
//    while (numColorAttachments >= wgpu_command_encoder_begin_render_pass_colorAttachments_desc.length)
    var attachments = wgpu_command_encoder_begin_render_pass_colorAttachments[numColorAttachments];
    ...
    for(var ai = 0; ai < numColorAttachments; ++ai) {
      ...
      if (HEAPU32[colorAttachmentsIdx]) {
        attachments[ai] = wgpu_command_encoder_begin_render_pass_colorAttachments_desc[ai];
...
/*
  ...
  */
    wgpu_command_encoder_begin_render_pass_desc['colorAttachments'] = attachments;
    wgpu_command_encoder_begin_render_pass_desc['depthStencilAttachment'] = wgpuReadRenderPassDepthStencilAttachment(descriptor+5);
    wgpu_command_encoder_begin_render_pass_desc['occlusionQuerySet'] = wgpu[HEAPU32[descriptor+14]];
    wgpu_command_encoder_begin_render_pass_desc['maxDrawCount'] = maxDrawCount || void 0;
    wgpu_command_encoder_begin_render_pass_desc['timestampWrites'] = wgpuReadTimestampWrites(descriptor+15);

    {{{ wdebugdir('desc', '`GPUCommandEncoder.beginRenderPass() with descriptor:`') }}};
    return wgpuStore(wgpu[commandEncoder]['beginRenderPass'](wgpu_command_encoder_begin_render_pass_desc));
  },
  ...
  $wgpu_queue_submit_one_and_destroy_array: [null],

  wgpu_queue_submit_one_and_destroy__deps: ['wgpu_object_destroy', '$wgpu_queue_submit_one_and_destroy_array'],
  wgpu_queue_submit_one_and_destroy: function(queue, commandBuffer) {
    ...
    wgpu_queue_submit_one_and_destroy_array[0] = wgpu[commandBuffer];
    wgpu[queue]['submit'](wgpu_queue_submit_one_and_destroy_array);
