...
//  $GPUTextureSampleTypes: [, 'float', 'unfilterable-float', 'depth', 'sint', 'uint'],
  ...
  wgpu_device_create_shader_module: function(device, descriptor) {
    ...
    return wgpuStoreAndSetParent(wgpu[device]['createShaderModule']({ 'code': UTF8ToString(HEAPU32[descriptor>>2]) }), wgpu[device]);
#endif
  },
  ...
  wgpu_command_encoder_begin_render_pass_1color_0depth: function(commandEncoder, descriptor) {
    ...
    var colorAttachmentsIdx = HEAPU32[descriptor+1] >> 2,
      colorAttachmentsIdxDbl = colorAttachmentsIdx + 4 >> 1; // Alias the view for HEAPF64.
