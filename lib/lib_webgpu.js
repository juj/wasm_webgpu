...
mergeInto(LibraryManager.library, {
  ...
  wgpu_canvas_context_configure_swap_chain: function(canvasContext, descriptor) {
    ...
    var gpuSwapChainDescriptor = {
      'device': wgpu[HEAPU32[descriptor]],
      'format': _wgpuStrings[HEAPU32[descriptor+1]],
  ...
  wgpu_device_create_render_pipeline: function(device, descriptor) {
    ...
    while(numVertexBuffers--) {
      ...
      vertexBuffers.push({
        'arrayStride': wgpuReadI53FromU64HeapIdx(vertexBuffersIdx),
        'stepMode': _wgpuStrings[HEAPU32[vertexBuffersIdx+2]],
  ...
  wgpu_queue_write_texture: function(queue, destination, data, bytesPerBlockRow, blockRowsPerImage, writeWidth, writeHeight, writeDepthOrArrayLayers) { // TODO: this function is untested. Write a test case
    ...
    wgpu[queue]['writeTexture'](wgpu[destination], HEAPU8, { offset: data, bytesPerRow: bytesPerBlockRow, rowsPerImage: blockRowsPerImage }, data, [writeWidth, writeHeight, writeDepthOrArrayLayers]);
  },
  ...
  wgpu_queue_copy_external_image_to_texture: function(queue, source, destination, copyWidth, copyHeight, copyDepthOrArrayLayers) { // TODO: this function is untested. Write a test case
    ...
    wgpu[queue]['copyExternalImageToTexture'](wgpu[source], wgpu[destination], [copyWidth, copyHeight, copyDepthOrArrayLayers]);
  },
});
