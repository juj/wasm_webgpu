...
mergeInto(LibraryManager.library, {
  ...
  wgpu_adapter_request_device_async: function(adapter, descriptor, deviceCallback, userData) {
    ...
    adapter['requestDevice'](descriptor).then((device) => {
      ...
      // TODO Remove || device['defaultQueue'] part when browsers update to new spec
      if (device['defaultQueue']) console.error('Browser implements old spec with GPUDevice.defaultQueue!');
      wgpuStore(device['queue'] || device['defaultQueue']);
...
//    {{{ wassert('wgpu[device]["adapter"]'); }}}
  ...
  wgpu_device_get_default_queue: function(device) {
    ...
    {{{ wassert('wgpu[device]'); }}}
    {{{ wassert('wgpu[device].wid == device', "GPUDevice has lost its wid member field!"); }}}
    {{{ wassert('(wgpu[device]["queue"] || wgpu[device]["defaultQueue"]).wid', "GPUDevice.defaultQueue must have been assigned an ID in function wgpu_adapter_request_device!"); }}}
      // TODO Remove  || wgpu[device]['defaultQueue'] part above and below when browsers update to new spec
    return (wgpu[device]['queue'] || wgpu[device]['defaultQueue']).wid;
  },
