...
mergeInto(LibraryManager.library, {
  ...
  $GPUTextureAndVertexFormats: "wgpuDecodeStrings('r8Z r8V r8uX8sXPuXPsXPYT8ZT8VT8uXg8sXQuXQsXQYTPuXgPsXgPYTK8ZTK8ZMTK8VTK8uXgK8sint bgra8Z bgra8ZMTb9e5uYTb10a2ZT11b10uYTQuXgQsXgQYTKPuXgKPsXgKPYTKQuXgKQsXgKQY O8RPZRARA-O8RQYN1FK-ZN1FK-ZMN2FK-ZN2FK-ZMN3FK-ZN3FK-ZMN4-r-ZN4-r-VN5F-ZN5F-VN6hFb-uYN6hFb-YN7FK-ZN7FK-ZMR24Z-O8RQY-O8S8BS8CU8BU8JZ8IZ8JV8IV8CSPBSPCUPBUPJZPIZPJVPIVPJYPIYPJYQ YQIYQx3 YQCSQSQBSQx3SQCUQUQBUQx3UQC','unorm|float|int r|-rgba-A|snorm| sint| rg| uint| depth|32|16|stencil| bc|-srgb|uCg|ba|x4 |x2 |sC|GP8|-rg| bgra8A|BG|x4|x2|24plus')",

// wgpuFeatures: 'depth-clamping depth24unorm-stencil8 depth32float-stencil8 pipeline-statistics-query texture-compression-bc timestamp-query'.split(' '),
  wgpuFeatures__deps: ['$wgpuDecodeStrings'],
  wgpuFeatures: "wgpuDecodeStrings('C-clamping C24unormB C32floatB pipeline-statisticsA texture-compression-bc timestampA','depth|-stencil8|-query').slice(1)",
...
//  wgpu32BitLimitNames: ['maxTextureDimension1D', 'maxTextureDimension2D', 'maxTextureDimension3D', 'maxTextureArrayLayers', 'maxBindGroups', 'maxDynamicUniformBuffersPerPipelineLayout', 'maxDynamicStorageBuffersPerPipelineLayout', 'maxSampledTexturesPerShaderStage', 'maxSamplersPerShaderStage', 'maxStorageBuffersPerShaderStage', 'maxStorageTexturesPerShaderStage', 'maxUniformBuffersPerShaderStage', 'minUniformBufferOffsetAlignment', 'minStorageBufferOffsetAlignment', 'maxVertexBuffers', 'maxVertexAttributes', 'maxVertexBufferArrayStride', 'maxInterStageShaderComponents', 'maxComputeWorkgroupStorageSize', 'maxComputeInvocationsPerWorkgroup', 'maxComputeWorkgroupSizeX', 'maxComputeWorkgroupSizeY', 'maxComputeWorkgroupSizeZ'],
  ...
  wgpu32BitLimitNames: "wgpuDecodeStrings('max;81D>;82D>;83D>;ArrayLayers>BindGroups>Dynamic7@sP4eLayout>Dynamic:@sP4eLayout>Sampled;?<>Sampler?<>:@?<>:;?<>7@?< min7@OffsetAlignment min:@OffsetAlignment>Vertex@s>VertexAttributes>Vertex@ArrayStride>Inte<ShaderComponents>9=:Size>9InvocationsPer=>9=SizeX>9=SizeY>9=SizeZ','Buffer|sPerShade| max|Workgroup|rStage|Texture|Storage|Compute|Dimension|Uniform|6Verte|;7Size|erPipelin',52).slice(1)",
...
//  $GPUBlendFactors: [, 'zero', 'one', 'src', 'one-minus-src', 'src-alpha', 'one-minus-src-alpha', 'dst', 'one-minus-dst', 'dst-alpha', 'one-minus-dst-alpha', 'src-alpha-saturated', 'constant', 'one-minus-constant'],
  $GPUBlendFactors: "wgpuDecodeStrings('zeroBne CBGC CFBGCF EBGE EFBGEF CF-saturated DBGD','ne-minus-|-alpha|dst|constant|src| o|FA')",
...
//  $GPUTextureAspects: [, 'all', 'stencil-only', 'depth-only'],
  ...
  $wgpuReadGpuBlendComponent__deps: ['$GPUBlendFactors'],
  $wgpuReadGpuBlendComponent: function(idx) {
    {{{ wassert('idx != 0'); }}}
    return {
      'operation': [, 'add', 'subtract', 'reverse-subtract', 'min', 'max'][HEAPU32[idx]],
  ...
  $wgpuReadRenderPipelineDescriptor__deps: ['$readI53FromU64', '$wgpuReadGpuStencilFaceState', '$wgpuReadGpuBlendComponent', '$wgpuReadI53FromU64HeapIdx', '$wgpuReadConstants', '$GPUIndexFormats', '$GPUTextureAndVertexFormats', '$GPUCompareFunctions'],
  $wgpuReadRenderPipelineDescriptor: function(descriptor) {
    ...
    desc = {
      ...
      'primitive': {
        'topology': [, 'point-list', 'line-list', 'line-strip', 'triangle-list', 'triangle-strip'][HEAPU32[primitiveIdx]],
  ...
  wgpu_device_create_query_set__deps: ['$wgpuStoreAndSetParent'],
  wgpu_device_create_query_set: function(device, descriptor) { // TODO: this function is untested. Write a test case
    ...
    while(numPipelineStatistics--) {
      pipelineStatistics.push([, 'vertex-shader-invocations', 'clipper-invocations', 'clipper-primitives-out', 'fragment-shader-invocations', 'compute-shader-invocations'][HEAPU32[pipelineStatisticsIdx++]]);
    }
  ...
  wgpu_device_create_bind_group_layout__deps: ['$wgpuStoreAndSetParent', '$GPUTextureViewDimensions', '$GPUTextureAndVertexFormats'],
  wgpu_device_create_bind_group_layout: function(device, entries, numEntries) { // TODO: this function is untested. Write a test case
    ...
    while(numEntries--) {
      ...
      if (type == 1/*WGPU_BIND_GROUP_LAYOUT_TYPE_BUFFER*/) {
        entry['buffer'] = {
          'type': [, 'uniform', 'storage', 'read-only-storage'][HEAPU32[entries]],
        ...
        entry['sampler'] = {
          'type': [, 'filtering', 'non-filtering', 'comparison'][HEAPU32[entries]]
        };
      } else if (type == 3/*WGPU_BIND_GROUP_LAYOUT_TYPE_TEXTURE*/) {
        entry['texture'] = {
          'sampleType': [, 'float', 'unfilterable-float', 'depth', 'sint', 'uint'][HEAPU32[entries]],
  ...
  wgpu_render_encoder_base_set_index_buffer: function(passEncoder, buffer, indexFormat, offset, size) { // TODO: this function is untested. Write a test case
    ...
    if (size >= 0) wgpu[passEncoder]['setIndexBuffer'](wgpu[buffer], GPUIndexFormats[indexFormat], offset, size);
    else wgpu[passEncoder]['setIndexBuffer'](wgpu[buffer], GPUIndexFormats[indexFormat], offset);
  },
  ...
  wgpu_render_encoder_base_set_vertex_buffer: function(passEncoder, slot, buffer, offset, size) {
    ...
    // Awkward API polymorphism: must omit size parameter altogether (instead of specifying e.g. -1 for whole buffer)
    if (size >= 0) wgpu[passEncoder]['setVertexBuffer'](slot, wgpu[buffer], offset, size);
    else wgpu[passEncoder]['setVertexBuffer'](slot, wgpu[buffer], offset);
  },
