...
//////////////////////////////////////////////////////////////////
  ...
  wgpu_canvas_context_configure: function(canvasContext, config) {
    ...
    let desc = {
      ...
      'viewFormats': wgpuReadArrayOfWgpuObjects({{{readPtr('config+4')}}}, HEAPU32[config+3]),
  ...
  $wgpuReadShaderModuleCompilationHints: function(index) {
    let numHints = HEAP32[index+2],
      hints = [],
      hintsIndex = {{{ readPtr('index') }}},
      layout;
    {{{ ptrToIdx('hintsIndex', 2) }}};
    {{{ wassert('numHints >= 0'); }}}
    while(numHints--) {
      ...
      hints.push({
        'entryPoint': utf8({{{ readPtr('hintsIndex') }}}),
  ...
  wgpu_device_create_query_set: function(device, descriptor) {
    ...
    let pipelineStatistics = [],
      ...
      pipelineStatisticsIdx = {{{ shiftPtr('HEAPU32[descriptor+3]', 2) }}}; // TODO: remove, pipelineStatistics no longer exists.
    ...
    let desc = {
      ...
      'pipelineStatistics': pipelineStatistics // TODO: remove, pipelineStatistics no longer exists.
    };
  ...
  wgpu_device_create_texture: function(device, descriptor) {
    ...
    let desc = {
      'viewFormats': wgpuReadArrayOfWgpuObjects({{{ readPtr('descriptor') }}}, HEAPU32[descriptor+2]),
