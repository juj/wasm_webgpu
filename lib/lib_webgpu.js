#if EMSCRIPTEN_VERSION.split('.').map(parseFloat)[0] < 3 || (EMSCRIPTEN_VERSION.split('.').map(parseFloat)[0] == 3 && EMSCRIPTEN_VERSION.split('.').map(parseFloat)[1] < 1) || (EMSCRIPTEN_VERSION.split('.').map(parseFloat)[0] == 3 && EMSCRIPTEN_VERSION.split('.').map(parseFloat)[1] == 1 && EMSCRIPTEN_VERSION.split('.').map(parseFloat)[2] < 35)
...
{{{
  globalThis.wassert = function(condition) {
    if (ASSERTIONS || globalThis.WEBGPU_DEBUG) return `assert(${condition}, "assert(${condition.replace(/"/g, "'")}) failed!");`;
    else return '';
  };
  globalThis.wdebuglog = function(condition) {
    return globalThis.WEBGPU_DEBUG ? `console.log(${condition});` : '';
  }
  globalThis.wdebugwarn = function(condition) {
    return globalThis.WEBGPU_DEBUG ? `console.warn(${condition});` : '';
  }
  globalThis.wdebugerror = function(condition) {
    return globalThis.WEBGPU_DEBUG ? `console.error(${condition});` : '';
  }
  globalThis.wdebugdir = function(condition, desc) {
    return globalThis.WEBGPU_DEBUG ? (typeof desc === 'undefined' ? `console.dir(${condition});` : `console.log(${desc}); console.dir(${condition});`) : '';
  }
...
let api = {
  ...
  $wgpuStore: function(object) {
    if (object) {
      ...
      {{{ wdebugdir('object', '`Stored WebGPU object with ID ${wgpuIdCounter}:`') }}};
...
#if ASYNCIFY == 2
    var func = WebAssembly.promising({{{ makeDynCall('idp', 'cb') }}});
#else
    var func = {{{ makeDynCall('idp', 'cb') }}};
#endif
...
#endif
        func(timeStamp, userData)) requestAnimationFrame(tick);
    }
...
//wgpuFeatures: ['depth-clip-control' (0), 'depth32float-stencil8' (1), 'texture-compression-bc' (2), 'texture-compression-etc2' (3), 'texture-compression-astc' (4), 'timestamp-query' (5), 'indirect-first-instance' (6), 'shader-f16' (7), 'rg11b10ufloat-renderable' (8), 'bgra8unorm-storage' (9), 'float32-filterable' (10), 'clip-distances' (11)],
  wgpuFeatures: "wgpuDecodeStrings('A-Ccontrol A32E-Dencil8GbcGetc2GaDc timeDamp-query indirect-firD-inB shader-f16 rg11b10uE-rendFbgra8unorm-Dorage E32-filtFCdiBs', ' texture-compression-|erable |float|st|clip-|Dance|depth').slice(1)",
...
//////////////////////////////////////////////////////////////////
...
#if EMSCRIPTEN_VERSION.split('.').map(parseFloat)[0] > 3 || (EMSCRIPTEN_VERSION.split('.').map(parseFloat)[0] == 3 && EMSCRIPTEN_VERSION.split('.').map(parseFloat)[1] > 1) || (EMSCRIPTEN_VERSION.split('.').map(parseFloat)[0] == 3 && EMSCRIPTEN_VERSION.split('.').map(parseFloat)[1] == 1 && EMSCRIPTEN_VERSION.split('.').map(parseFloat)[2] >= 57)
  ...
  wgpu_adapter_request_device_async: function(adapter, descriptor, deviceCallback, userData) {
    ...
    {{{ wassert('_wgpuFeatures.length == 12'); }}}
...
#if ASYNCIFY
  wgpu_adapter_request_device_sync__deps: ['$wgpuStore', '$wgpuStoreAsyncifiedOp', 'wgpuFeatures', 'wgpu32BitLimitNames', 'wgpu64BitLimitNames', '$wgpuReadI53FromU64HeapIdx', '_wgpuNumAsyncifiedOperationsPending', '$wgpu_async'],
