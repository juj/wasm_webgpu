...
//    {{{ wassert('wgpu[device]["adapter"]'); }}}
  ...
  wgpu_buffer_read_mapped_range: function(gpuBuffer, offset, size, dst) {
    {{{ wdebuglog('`wgpu_buffer_read_mapped_range(gpuBuffer=${gpuBuffer}, offset=${offset}, size=${size}, dst=${dst})`'); }}}
    ...
    HEAPU8.set(new Uint8Array(wgpu[gpuBuffer]['getMappedRange'](), offset, size), dst);
  },

  wgpu_buffer_write_mapped_range: function(gpuBuffer, offset, size, src) {
    {{{ wdebuglog('`wgpu_buffer_write_mapped_range(gpuBuffer=${gpuBuffer}, offset=${offset}, size=${size}, src=${src})`'); }}}
    {{{ wassert('wgpu[gpuBuffer]'); }}}
    {{{ wassert('wgpu[gpuBuffer].wid == gpuBuffer', "GPUBuffer has lost its wid member field!"); }}}
    {{{ wassert('src || size == 0'); }}}
    {{{ wassert('offset >= 0'); }}}
    ...
    new Uint8Array(wgpu[gpuBuffer]['getMappedRange']()).set(new Uint8Array(buffer, src, size), offset);
  },
  ...
  wgpu_render_pass_encoder_set_vertex_buffer: function(passEncoder, slot, buffer, offset, size) {
    {{{ wdebuglog('`wgpu_render_pass_encoder_set_vertex_buffer(passEncoder=${passEncoder}, slot=${slot}, buffer=${buffer}, offset=${offset}, size=${size})`'); }}}
    {{{ wassert('wgpu[passEncoder]'); }}}
    {{{ wassert('wgpu[passEncoder].wid == passEncoder', "GPUPassEncoder has lost its wid member field!"); }}}
    {{{ wassert('wgpu[buffer]'); }}}
