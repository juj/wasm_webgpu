...
//  $GPUTextureSampleTypes: [, 'float', 'unfilterable-float', 'depth', 'sint', 'uint'],
  ...
  wgpu_adapter_request_device_async: function(adapter, descriptor, deviceCallback, userData) {
    ...
    }, 'GPUAdapter.requestDevice() with desc')).then(cb).catch(cb);
  },
  ...
  wgpu_device_create_shader_module: function(device, descriptor) {
    ...
    return wgpuStoreAndSetParent(wgpu[device]['createShaderModule'](
      debugDir({ 'code': UTF8ToString(HEAPU32[{{{ shiftPtr('descriptor', 2) }}}]) }, 'device.createShaderModule() with desc')
      ), wgpu[device]);
  },
  ...
  wgpu_device_create_buffer: function(device, descriptor) {
    ...
    var buffer = device['createBuffer'](debugDir({
    ...
    buffer.isBuffer = 1;
    return wgpuStoreAndSetParent(debugDir(buffer, 'returned'), device);
  },
  ...
  wgpu_device_create_render_pipeline: function(device, descriptor) {
    ...
    return wgpuStoreAndSetParent(
      debugDir(wgpu[device]['createRenderPipeline'](
        debugDir(
          wgpuReadRenderPipelineDescriptor(descriptor), 'GPUDevice.createRenderPipeline() with desc')
        ),
        'returned'
      ), wgpu[device]);
  },
  ...
  wgpu_device_create_command_encoder: function(device, descriptor) {
    ...
    return wgpuStoreAndSetParent(debugDir(wgpu[device]['createCommandEncoder'](
      debugDir(
        descriptor ? {
  ...
  wgpu_device_create_render_bundle_encoder: function(device, descriptor) { // TODO: this function is untested. Write a test case
    ...
    return wgpuStoreAndSetParent(debugDir(
  ...
  wgpu_device_create_query_set: function(device, descriptor) { // TODO: this function is untested. Write a test case
    ...
    return wgpuStoreAndSetParent(debugDir(
      device['createQuerySet'](debugDir(
  ...
  wgpu_device_create_pipeline_layout: function(device, layouts, numLayouts) { // TODO: this function is untested. Write a test case
    ...
    return wgpuStoreAndSetParent(debugDir(
      device['createPipelineLayout'](
        debugDir({ 'bindGroupLayouts': e }, 'GPUDevice.createPipelineLayout() with desc')),
      'returned'),
      device);
  },
  ...
  wgpu_device_create_bind_group: function(device, layout, entries, numEntries) {
    ...
    return wgpuStoreAndSetParent(debugDir(device['createBindGroup'](debugDir({
        'layout': wgpu[layout],
        'entries': e
      }, 'GPUDevice.createBindGroup() with desc')), 'returned'), device);
  },
  ...
  wgpu_device_create_compute_pipeline: function(device, computeModule, entryPoint, constants, numConstants) { // TODO: this function is untested. Write a test case
    ...
    return wgpuStoreAndSetParent(debugDir(device['createComputePipeline'](debugDir({
        'compute': {
          'module': wgpu[computeModule],
          'entryPoint': UTF8ToString(entryPoint),
          'constants': wgpuReadConstants(constants, numConstants)
        }
      }, 'GPUDevice.createComputePipeline() with desc')), 'returned'), device);
  },
  ...
  wgpu_texture_create_view: function(texture, descriptor) {
    ...
    {{{ ptrToIdx('descriptor', 2); }}}
    return wgpuStoreAndSetParent(debugDir(wgpu[texture]['createView'](debugDir(descriptor ? {
      ...
      } : void 0), 'GPUTexture.createView() with desc'), 'returned'), wgpu[texture]);
  },
