...
mergeInto(LibraryManager.library, {
  ...
  wgpu_canvas_get_webgpu_context__deps: ['$wgpuAllocId', '$wgpuStore'],
...
//  $GPUTextureAspects: [, 'all', 'stencil-only', 'depth-only'],
  ...
  navigator_gpu_request_adapter_async__deps: ['$wgpuAllocId'],
  navigator_gpu_request_adapter_async: function(options, adapterCallback, userData) {
    ...
    if (gpu) {
      ...
      function cb(adapter) {
        ...
        {{{ makeDynCall('vii', 'adapterCallback') }}}(adapter && id, userData);
      }
  ...
  navigator_gpu_request_adapter_async_simple__deps: ['$wgpuAllocId'],
  ...
  wgpu_adapter_request_device_async: function(adapter, descriptor, deviceCallback, userData) {
    ...
    function cb(device) {
      ...
      {{{ makeDynCall('vii', 'deviceCallback') }}}(device && wgpuStore(device), userData);
    }
  ...
  wgpu_command_encoder_begin_render_pass_1color_0depth: function(commandEncoder, descriptor) {
    {{{ wdebuglog('`wgpu_command_encoder_begin_render_pass_1color(commandEncoder=${commandEncoder}, descriptor=${descriptor})`'); }}}
    {{{ wassert('wgpu[commandEncoder] instanceof GPUCommandEncoder'); }}}
    {{{ wassert('descriptor != 0'); }}}
    {{{ wassert('descriptor % 4 == 0'); }}} // Must be aligned at uint32_t boundary
    ...
    {{{ wassert('wgpu[HEAPU32[descriptor+2]]'); }}} // Must be passing no depth-stencil target.
