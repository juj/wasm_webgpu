...
mergeInto(LibraryManager.library, {
  ...
  wgpu_canvas_context_configure: function(canvasContext, config) {
    ...
    var size = [HEAP32[config+5], HEAP32[config+6], HEAP32[config+7]];
  ...
  wgpu_canvas_context_get_current_texture: function(canvasContext) {
    ...
    var texture = canvasContext['getCurrentTexture']();
    {{{ wassert('texture'); }}}
  ...
  wgpu_device_create_shader_module: function(device, descriptor) {
    ...
    {{{ wassert('descriptor % 4 == 0'); }}} // Must be aligned at uint32_t boundary
    device = wgpu[device];
#if ASSERTIONS // TODO: #if WEBGPU_DEBUG
    ...
    return wgpuStoreAndSetParent(device['createShaderModule']({ 'code': UTF8ToString(HEAPU32[descriptor>>2]) }), device);
#endif
  },
  ...
  $wgpuReadConstants: function(constants, numConstants) {
    ...
    constants >>= 2;
    while(numConstants--) {
      c[UTF8ToString(HEAPU32[constants])] = HEAPF64[constants + 2 >> 1];
      constants += 4;
    }
    return c;
  },
  ...
  wgpu_texture_create_view: function(texture, descriptor) {
    ...
    return wgpuStoreAndSetParent(t['createView'](desc), t);
#endif
  },
