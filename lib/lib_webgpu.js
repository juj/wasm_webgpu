...
mergeInto(LibraryManager.library, {
  ...
  wgpu_canvas_context_get_preferred_format__deps: ['$GPUTextureFormats'],
  wgpu_canvas_context_get_preferred_format: function(canvasContext, adapter) {
    ...
    {{{ wassert('GPUTextureFormats.includes(wgpu[canvasContext].getPreferredFormat(wgpu[adapter]))'); }}}
    return GPUTextureFormats.indexOf(wgpu[canvasContext]['getPreferredFormat'](wgpu[adapter]));
  },

  $GPUPredefinedColorSpaces: [, 'srgb'],

  wgpu_canvas_context_configure__deps: ['$GPUTextureFormats', '$GPUPredefinedColorSpaces'],
  wgpu_canvas_context_configure: function(canvasContext, config) {
    ...
    var desc = {
      ...
      'format': GPUTextureFormats[HEAPU32[config+1]],
  ...
  $wgpuReadRenderPipelineDescriptor__deps: ['$readI53FromU64', '$wgpuReadGpuStencilFaceState', '$wgpuReadGpuBlendComponent', '$wgpuReadI53FromU64HeapIdx', '$wgpuReadConstants', '$GPUIndexFormats', '$GPUTextureFormats', '$GPUCompareFunctions'],
  $wgpuReadRenderPipelineDescriptor: function(descriptor) {
    ...
    while(numVertexBuffers--) {
      ...
      while(numAttributes--) {
        attributes.push({
          ...
          'format': [,'uint','sint','unorm','snorm','float'][HEAPU32[attributesIdx+3]&7]
                  + [, '8x2', '8x4', '16x2', '16x4', '32', '32x2', '32x3', '32x4'][HEAPU32[attributesIdx+3]>>4]
        });
        attributesIdx += 4;
      }
      vertexBuffers.push({
    ...
    while(numTargets--) {
      targets.push({
        'format': GPUTextureFormats[HEAPU32[targetsIdx]],
        'blend': {
    ...
    if (depthStencilFormat) desc['depthStencil'] = {
        'format': GPUTextureFormats[depthStencilFormat],
  ...
  wgpu_device_create_texture__deps: ['$wgpuStoreAndSetParent', '$GPUTextureFormats'],
  wgpu_device_create_texture: function(device, descriptor) {
    ...
    var desc = {
      ...
      'format': GPUTextureFormats[HEAPU32[descriptor+6]],
  ...
  wgpu_device_create_bind_group_layout__deps: ['$wgpuStoreAndSetParent', '$GPUTextureViewDimensions', '$GPUTextureFormats'],
    ...
    while(numEntries--) {
      ...
      if (type == 1/*WGPU_BIND_GROUP_LAYOUT_TYPE_BUFFER*/) {
        ...
        entry['storageTexture'] = {
          ...
          'format': GPUTextureFormats[HEAPU32[entries+1]],
  ...
  wgpu_texture_create_view__deps: ['$wgpuStoreAndSetParent', '$GPUTextureFormats', '$GPUTextureViewDimensions', '$GPUTextureAspects'],
...
#if ASSERTIONS
    var desc = descriptor ? {
      'format': GPUTextureFormats[HEAPU32[descriptor]],
