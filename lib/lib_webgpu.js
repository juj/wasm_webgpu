...
mergeInto(LibraryManager.library, {
  ...
  // TODO: Work around Chrome BUG: GPUPresentationContext is not defined, but it is used under old name GPUCanvasContext
  wgpu_is_presentation_context: function(o) { return wgpu[o] instanceof GPUCanvasContext; },
//  wgpu_is_presentation_context: function(o) { return wgpu[o] instanceof GPUPresentationContext; },
  ...
  wgpu_device_pop_error_scope_async: function(device, callback, userData) {
    ...
    function dispatchErrorCallback(error) {
      var stackTop, errorMessage, msg = error && error['message'], errorType = error ? (error instanceof GPUValidationError
      ...
      if (msg) {
        ...
        stringToUTF8(msg, errorMessage, len);
      }
      {{{ makeDynCall('viiii', 'callback') }}}(device, errorType, errorMessage, userData);
  ...
  wgpu_adapter_request_device_async__deps: ['wgpuFeatures'],
  wgpu_adapter_request_device_async: function(adapter, descriptor, deviceCallback, userData) {
    ...
    descriptor >>= 2;
    var requiredFeatures = [], requiredLimits = {}, id = wgpuAllocId(), v = HEAPU32[descriptor++];
    for(var i = 0; i < 32; ++i) {
      if (v & (1 << i)) requiredFeatures.push(_wgpuFeatures[i]);
    }
    ++descriptor;
    for(var fieldName of ['maxTextureDimension1D',
      ...
      if ((v = HEAPU32[descriptor++])) requiredLimits[fieldName] = v;
    }
  ...
  wgpu_device_create_bind_group: function(device, layout, entries, numEntries) { // TODO: this function is untested. Write a test case
    ...
    while(numEntries--) {
      ...
      if (resource.isBuffer) {
        ...
        if (size) resourceBinding['size'] = size; // TODO: awkward polymorphism. Would like to unconditionally set this.
