...
mergeInto(LibraryManager.library, {
  ...
  wgpu_canvas_context_get_preferred_format__deps: ['wgpuStrings'],
  wgpu_canvas_context_get_preferred_format: function(canvasContext, adapter) {
    ...
    {{{ wassert('_wgpuStrings.includes(wgpu[canvasContext].getPreferredFormat(wgpu[adapter]))'); }}}
    return _wgpuStrings.indexOf(wgpu[canvasContext]['getPreferredFormat'](wgpu[adapter]));
  },
  ...
  wgpu_canvas_context_configure: function(canvasContext, config) {
    ...
    var desc = {
      ...
      'colorSpace': _wgpuStrings[HEAPU32[config+3]],
      'compositingAlphaMode': _wgpuStrings[HEAPU32[config+4]],
  ...
  wgpu_canvas_context_get_current_texture: function(c) {
    ...
    {{{ wassert('!texture.derivedObjects || texture.derivedObjects.length < 1000'); }}}
    if (c != wgpu[1]) {
  ...
  navigator_gpu_request_adapter_async: function(options, adapterCallback, userData) {
    ...
    if (navigator['gpu']) {
      options >>= 2;
      var id = wgpuAllocId(),
        powerPreference = _wgpuStrings[HEAPU32[options]],
        opts = {};
      if (options) {
  ...
  $wgpuReadGpuBlendComponent: function(idx) {
    {{{ wassert('idx != 0'); }}}
    return {
      'operation': _wgpuStrings[HEAPU32[idx]],
      'srcFactor': _wgpuStrings[HEAPU32[idx+1]],
      'dstFactor': _wgpuStrings[HEAPU32[idx+2]]
    };
  },
  ...
  $wgpuReadRenderPipelineDescriptor__deps: ['$readI53FromU64', '$wgpuReadGpuStencilFaceState', '$wgpuReadGpuBlendComponent', '$wgpuReadI53FromU64HeapIdx', '$wgpuReadConstants'],
  ...
  wgpu_command_encoder_begin_render_pass: function(commandEncoder, descriptor) {
    ...
    return wgpuStore(wgpu[commandEncoder]['beginRenderPass'](descriptor));
#endif
  },
  ...
  wgpu_render_encoder_base_set_index_buffer: function(passEncoder, buffer, indexFormat, offset, size) { // TODO: this function is untested. Write a test case
    ...
    if (size >= 0) wgpu[passEncoder]['setIndexBuffer'](wgpu[buffer], _wgpuStrings[indexFormat], offset, size);
    else wgpu[passEncoder]['setIndexBuffer'](wgpu[buffer], _wgpuStrings[indexFormat], offset);
  },
