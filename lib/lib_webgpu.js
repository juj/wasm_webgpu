...
{{{
  ...
  globalThis.wdebugdir = function(condition) {
    return globalThis.WEBGPU_DEBUG ? `console.dir(${condition});` : '';
  }
...
let api = {
  ...
  wgpu_request_animation_frame__deps: ['_wgpuNumAsyncifiedOperationsPending'],
  wgpu_request_animation_frame: function(cb, userData) {
#if JSPI
    // If we have a current JSPI operation in flight, spin a no-op animation frame loop until the async operation finishes.
    return requestAnimationFrame(__wgpuNumAsyncifiedOperationsPending
      ? () => _wgpu_request_animation_frame(cb, userData)
      : (timeStamp) => WebAssembly.promising({{{ makeDynCall('idp', 'cb') }}})(timeStamp, userData));
#else
    return requestAnimationFrame((timeStamp) => {{{ makeDynCall('idp', 'cb') }}}(timeStamp, userData));
#endif
  },
...
#if !globalThis.WEBGPU_NO_BW_COMPAT
...
//////////////////////////////////////////////////////////////////
  ...
  wgpu_adapter_get_info: function(adapter, infoPtr) {
    ...
    {{{ wdebuglog('`GPUAdapter.info is a member with following parameters:`'); }}}
