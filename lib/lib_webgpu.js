...
//  $GPUTextureSampleTypes: [, 'float', 'unfilterable-float', 'depth', 'sint', 'uint'],
...
#if ASYNCIFY
  navigator_gpu_request_adapter_sync__deps: ['$wgpuStore', '$debugDir'],
  navigator_gpu_request_adapter_sync: function(options) {
    ...
    if (gpu) {
      ...
      return Asyncify.handleAsync(() => {
        return gpu['requestAdapter'](opts).then(cb).catch(cb);
      });
    }
    {{{ werror('`WebGPU is not supported by the current browser!`'); }}}
    // Implicit return EM_FALSE, WebGPU is not supported.
...
#if ASYNCIFY
  navigator_gpu_request_adapter_sync_simple__deps: ['$wgpuStore'],
  ...
  wgpu_adapter_or_device_get_limits__deps: ['wgpu32BitLimitNames', '$wgpuWriteU64HeapIdx'],
  wgpu_adapter_or_device_get_limits: function(adapterOrDevice, limits) {
    ...
    wgpuWriteU64HeapIdx(limits+1, l['maxUniformBufferBindingSize']);
    wgpuWriteU64HeapIdx(limits+3, l['maxStorageBufferBindingSize']);
  },
...
#if ASYNCIFY
  wgpu_adapter_request_device_sync__deps: ['$wgpuStore', 'wgpuFeatures', 'wgpu32BitLimitNames', '$wgpuReadI53FromU64HeapIdx'],
...
#if ASYNCIFY
  wgpu_adapter_request_device_sync_simple__deps: ['$wgpuStore'],
  ...
  wgpu_device_create_shader_module: function(device, descriptor) {
    ...
    return wgpuStoreAndSetParent(
      wgpu[device]['createShaderModule'](
        debugDir(
          {
            'code': UTF8ToString(HEAPU32[{{{ shiftPtr('descriptor', 2) }}}])
          },
  ...
  $wgpuReadI53FromU64HeapIdx: function(heap32Idx) {
    {{{ wassert('heap32Idx != 0'); }}}
#if WASM_BIGINT
    return Number(HEAPU32[heap32Idx>>1]);
#else
  ...
  $wgpuReadRenderPipelineDescriptor__deps: ['$wgpuReadGpuStencilFaceState', '$wgpuReadGpuBlendComponent', '$wgpuReadI53FromU64HeapIdx', '$wgpuReadConstants', '$GPUIndexFormats', '$GPUTextureAndVertexFormats', '$GPUCompareFunctions', '$GPUPrimitiveTopologys'],
  ...
  wgpu_device_create_bind_group_layout__deps: ['$wgpuReadI53FromU64HeapIdx', '$wgpuStoreAndSetParent', '$GPUBufferBindingTypes', '$GPUSamplerBindingTypes', '$GPUTextureSampleTypes', '$GPUTextureViewDimensions', '$GPUTextureAndVertexFormats'],
