...
{{{
  ...
  globalThis.replacePtrToIdx = function(ptr, shift) {
    ...
    if (ASSERTIONS || parseInt(globalThis.WEBGPU_DEBUG)) {
      s += `assert(((${ptr} ${shr} ${shift}) << ${shift}) >>> 0 == ${ptr} >>> 0);\n`;
    }
  ...
  globalThis.shiftIndex = function(index, shift) {
    ...
    else if (MAXIMUM_MEMORY <= 4*1024*1024*1024) return `${index} >>> ${shift}`;
    else                                         return `${index} / ${1 << shift}`;
  }
...
//  navigator_gpu_request_adapter_sync__sig: 'ip', // Iirc this would be needed for -sASYNCIFY=1 build mode, but this breaks -sMEMORY64=1 due to the detrimental automatic wrappers
  ...
  wgpu_adapter_or_device_get_info: function(adapterOrDevice, infoPtr) {
    ...
    var infoIdx = {{{ shiftPtr('infoPtr', 2) }}},
      ...
      adapterInfo = wgpu[adapterOrDevice]['adapterInfo'] || wgpu[adapterOrDevice]['info'];
...
#if MEMORY64
        msgPtr += BigInt(stringToUTF8(msg['message'], msgPtr, 2**32) + 1);
#else
...
#if MEMORY64
    let eIdx = {{{ shiftPtr('e', 3) }}};
    HEAPU64[eIdx] = stringToNewUTF8(error['name']);
    HEAPU64[eIdx + 1] = stringToNewUTF8(error['message']);
    HEAPU64[eIdx + 2] = stringToNewUTF8(error['reason']);
    {{{ makeDynCall('vipip', 'callback') }}}(device, e, /*pipeline handle=*/0, userData);
    _free(HEAPU64[eIdx + 2]);
    _free(HEAPU64[eIdx + 1]);
    _free(HEAPU64[eIdx]);
#else
    let eIdx = {{{ shiftPtr('e', 2) }}};
    HEAPU32[eIdx] = stringToNewUTF8(error['name']);
    HEAPU32[eIdx + 2] = stringToNewUTF8(error['message']);
    HEAPU32[eIdx + 4] = stringToNewUTF8(error['reason']);
    {{{ makeDynCall('vipip', 'callback') }}}(device, e, /*pipeline handle=*/0, userData);
    _free(HEAPU32[eIdx + 4]);
    _free(HEAPU32[eIdx + 2]);
    _free(HEAPU32[eIdx]);
#endif
    _free(e);
  },
  ...
  wgpu_pipeline_get_bind_group_layout: function(pipelineBase, index) {
    ...
    return wgpuStoreAndSetParent(wgpu[pipelineBase]['getBindGroupLayout'](index), wgpu[pipelineBase]);
  },
  ...
  wgpu_encoder_set_bind_group: function(encoder, index, /*nullable*/ bindGroup, dynamicOffsets, numDynamicOffsets) {
    ...
    var firefoxWorkaroundBuffer = new Uint8Array(HEAPU32.buffer, {{{ shiftPtr('dynamicOffsets', 2) }}}, numDynamicOffsets);
...
#if MIN_FIREFOX_VERSION != TARGET_NOT_SUPPORTED && (MEMORY64 || CAN_ADDRESS_2GB)
    // No Wasm4GB/Wasm64 support in Firefox: https://bugzilla.mozilla.org/show_bug.cgi?id=2022805
    var firefoxWorkaroundBuffer = new Uint8Array(HEAPU8.buffer, {{{ shiftPtr('data', 0) }}}, size);
...
#if MIN_FIREFOX_VERSION != TARGET_NOT_SUPPORTED && (MEMORY64 || CAN_ADDRESS_2GB)
    // No Wasm4GB/Wasm64 support in Firefox: https://bugzilla.mozilla.org/show_bug.cgi?id=2022805
    var firefoxWorkaroundBuffer = new Uint8Array(HEAPU8.buffer, {{{ shiftPtr('data', 0) }}}, bytesPerBlockRow*blockRowsPerImage);
  ...
  offscreen_canvas_post_to_pthread: function(offscreenCanvasId, pthread) {
    ...
    PThread.pthreads[{{{ wasm4GbShift('pthread') }}}].postMessage({
      'wgpuCanvas': wgpuOffscreenCanvases[offscreenCanvasId],
      'id': offscreenCanvasId,
    }, [wgpuOffscreenCanvases[offscreenCanvasId]]);
    delete wgpuOffscreenCanvases[offscreenCanvasId];
